...
def make_label(label_str, allow_NEI=True):
    lookup = {"SUPPORT": Label.SUPPORTS,
              "NOTENOUGHINFO":  Label.NEI,
              "REFUTE": Label.REFUTES}
...
class Corpus:
    ...
    def from_jsonl(cls, corpus_file):
        ...
        documents = []
        for entry in corpus:
            doc = Document(entry["cord_id"], entry["title"], entry["abstract"])
...
class GoldDataset:
    ...
    def _read_claims(self, data_file):
        ...
        for this_example in examples:
            ...
            #                       for doc in entry["cord_id"]]
            # assert len(entry["cited_docs"]) == len(entry["cord_id"])
            # del entry["cord_id"]

            entry = {
                "id": this_example["id"],
                "claim": this_example["claim"],
                "evidence": {
                    this_example["cord_id"]: 
                        #EvidenceAbstract(this_example["label"], )
                        [{"sentences": [item["sent_index"] for item in this_example["evidence_set"]], 
                        "label":this_example["label"]}]
                        #"evidence": {"23895668": [{"sentences": [2, 3], "label": "SUPPORT"}]}, 
                        #"evidence_set": [{"sent_index": 6, "type": "SUPPLEMENTARY"}, {"sent_index": 12, "type": "PRIMARY"}]
                    },
                "cited_docs": [self.corpus.get_document(this_example["cord_id"])],
                "release": self
            }
...
class Claim:
    ...
    id: str
    claim: str
    evidence: Dict[str, EvidenceAbstract]
